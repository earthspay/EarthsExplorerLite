...
import {libs} from '@earths/signature-generator';
...
const transformAssetScript = (currencyService, tx) => {
    return currencyService.get(tx.assetId).then(asset => {
        return Object.assign(copyMandatoryAttributes(tx), {
            script: tx.script,
            asset,
            fee: Money.fromCoins(tx.fee, Currency.EARTHS)
        })
    });
};
...
const transformLease = (currencyService, tx) => {
    return currencyService.get(tx.feeAssetId).then(feeCurrency => {
        return Object.assign(copyMandatoryAttributes(tx), {
            ...
            fee: Money.fromCoins(tx.fee, feeCurrency),
            amount: Money.fromCoins(tx.amount, Currency.EARTHS),
            status: tx.status
        })
    });
};
...
const transformExchange = (currencyService, tx) => {
    ...
    return Promise.all([
        ...
        return Object.assign(copyMandatoryAttributes(tx), {
            fee: Money.fromCoins(tx.fee, Currency.EARTHS),
            buyFee: Money.fromCoins(tx.buyMatcherFee, Currency.EARTHS),
            sellFee: Money.fromCoins(tx.sellMatcherFee, Currency.EARTHS),
            price,
            amount,
...
const transformOrder = (order, assetPair) => {
    return {
        ...
        fee: Money.fromCoins(order.matcherFee, Currency.EARTHS)
    };
};
